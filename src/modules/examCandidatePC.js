...
if (window.location.protocol === "http:") server = "http://re-coder.net/janus";
else server = "https://re-coder.net/janus";

export const runJanusPC = (studentId) => {
	Janus.init({
		debug: "all",
		callback: function () {
			janus = new Janus({
				...
				success: function () {
					...
					janus.attach({
						...
						error: function (error) {
							Janus.error(
								" --Janus-- WebCam Error attaching plugin...",
								error
							);
						},
						iceState: function (state) {
							Janus.log(
								" --Janus-- WebCam ICE state changed to " +
									state
							);
						},
						...
						onmessage: function (msg, jsep) {
							...
							if (event) {
								if (event === "joined") {
									...
									Janus.log(
										` --Janus-- WebCam Display Name : ${myId} 님이 Feed를 생성하였습니다.`
									);
									...
									for (var f in list) {
										...
										if (
											...
											Janus.log(
												` --Janus-- Mobile Display name : ${display} 님이 Feed를 생성하였습니다.`
											);
											// createAnswer 제작
											mobileFeed(id);
										}
									...
									alert(
										"The Janus room has been destroyed",
										function () {
						...
						onlocalstream: function (stream) {
							Janus.debug(
								" --Janus-- ::: WebCam Got a local stream :::",
								stream
							);
							// html tag 값 가져오기
	...
	janus.attach({
		...
		onmessage: function (msg, jsep) {
			...
			if (event) {
				if (event === "joined") {
					Janus.log(
						` --Janus-- Screen Display Name : ${screenId} 님이 Feed를 생성하였습니다.`
					);
 ...
 */
const joinTheRoom = (roomID, userId) => {
	let register = {
		request: "join",
		room: roomID,
		ptype: "publisher",
 ...
 */
const createTheRoom = (numOfCandidate) => {
	let create = {
		request: "create",
		require_pvtid: true,
		bitrate: 500000,
